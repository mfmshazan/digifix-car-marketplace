...
import { loginUser, saveToken, saveUser } from "../../src/api/auth";
...
export default function LoginScreen() {
  ...
  useEffect(() => {
    if (isUserLoaded && clerkUser) {
      // User is authenticated with Clerk, sync with backend
  ...
  const syncClerkUserWithBackend = async () => {
    ...
    try {
      // Get Clerk session token using getIdTokenClaims
      const idToken = await clerkUser.getIdTokenClaims();
      const sessionToken = idToken?.raw || idToken?.toString();

      if (sessionToken) {
        // Call backend to sync user and get JWT token
        const tokenResponse = await fetch(
          "http://10.19.40.60:3000/api/auth/clerk/google/callback",
          {
            method: "POST",
            headers: {
        ...
        if (tokenData.success && tokenData.data) {
          await saveToken(tokenData.data.token);
          await saveUser(tokenData.data.user);

          Alert.alert("Success", "Google sign-in successful!");

          if (tokenData.data.user.role === "SALESMAN") {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Use Clerk's OAuth sign in with redirect
